...
def index(request):
    ...
    # Today's profit
    today_items = SaleItem.objects.filter(sale__sale_date__date=today, sale__status='completed').select_related('product')
    today_profit = sum((item.unit_price - item.product.purchase_price) * item.quantity for item in today_items)
    ...
    in_stock_count = Product.objects.filter(
        is_active=True, stock_quantity__gt=5
    ).count()
    top_low_stock = Product.objects.filter(is_active=True, stock_quantity__lte=5).order_by('stock_quantity')[:5]
    ...
    # ─── Recent Sales ───
    recent_sales = Sale.objects.select_related('customer', 'created_by').order_by('-created_at')[:8]
